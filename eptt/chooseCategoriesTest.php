...
//barkod ile ürün aratıp bulduğumuzda, bize döndürülen üründe sadece kategori adı var.
      ...
      if( o.name === name ){//eğer objenin adı istediğimiz isimse objeyi döndür.
          for (i in o.children.Category) {
           id+= "<option>"+o.children.Category[i].name+"</option>";
          }
          return id;
      }
    var result, p; 
...
<?php
   ...
   // $cate= $EPTT->KategoriListesi(); 
    $cate= $EPTT->GetCategoryTree(0);
    $a = current($cate);
    $array=json_encode($a);
